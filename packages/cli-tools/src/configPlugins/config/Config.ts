...
export function getConfig(
  ...
  const rawStaticConfig = paths.staticConfigPath
    ? getStaticConfig(paths.staticConfigPath)
    : null;
  ...
  if (config.dynamicConfigPath) {
    ...
    */
    ...
    if (!writeOptions.dryRun) {
      await JsonFile.writeAsync(config.staticConfigPath, outputConfig, {
        json5: false,
      });
    }
...
export function getNameFromConfig(
  ...
  const appName =
    exp.displayName || appManifest.displayName || appManifest.name;
...
export function getDefaultTarget(
  ...
  if (
    ...
    semver.lt(exp.sdkVersion, '37.0.0')
  ) {
    return 'managed';
  }
...
export function getProjectConfigDescriptionWithPaths(
  ...
  if (projectConfig.dynamicConfigPath) {
    const relativeDynamicConfigPath = path.relative(
      projectRoot,
      projectConfig.dynamicConfigPath,
    );
