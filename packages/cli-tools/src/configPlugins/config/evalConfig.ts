  ...
  try {
    ...
    if (location) {
      ...
      const codeFrame = codeFrameColumns(
        contents,
        {start: parsedError.loc},
        {highlightCode: true},
      );
      parsedError.codeFrame = codeFrame;
      parsedError.message += `\n${codeFrame}`;
    } else {
...
function isSyntaxError(error: any): error is SyntaxError {
  return (
    error instanceof SyntaxError || error.constructor.name === 'SyntaxError'
  );
}
...
export function resolveConfigExport(
  ...
  if (result instanceof Promise) {
    throw new ConfigError(
      `Config file ${configFile} cannot return a Promise.`,
      'INVALID_CONFIG',
    );
  }
